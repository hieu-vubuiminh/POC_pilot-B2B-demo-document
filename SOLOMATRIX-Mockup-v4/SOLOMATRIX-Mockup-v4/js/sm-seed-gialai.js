...
(function (global) {
  ...
  /* ─── v4: thêm cho onboarding — xem PLAN-V4 B.3/B.5, CHOT P2/P3 ─── */

  // Mã suất Chương trình GL26-XXXX-XXXX [P3] — sinh bằng LCG RIÊNG để không xô
  // dịch chuỗi ngẫu nhiên của genRevenue (mở lại vẫn ra đúng số cũ).
  const rSuat = rng(20260817);
  function maSuat() {
    const AB = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789'; // bỏ I, O, 0, 1 — dễ đọc nhầm khi hộ gõ tay
    let a = '', b = '';
    for (let i = 0; i < 4; i++) a += AB[Math.floor(rSuat() * AB.length)];
    for (let i = 0; i < 4; i++) b += AB[Math.floor(rSuat() * AB.length)];
    return 'GL26-' + a + '-' + b;
  }

  // Thuế sàn đã khấu trừ nộp thay [Q-019 · NĐ 117/2025 Đ11 k4] trên đơn kênh sàn —
  // số mô phỏng ~1,5% giá trị đơn (chờ Q-032). Chỉ THÊM trường, không đổi số cũ.
  function ganThueSan(orders) {
    orders.forEach(o => {
      if (['shopee', 'tiktok', 'lazada'].indexOf(o.channel) >= 0) {
        o.thueSanDaNop = Math.round(o.total * 0.015);
      }
    });
    return orders;
  }

  // Khối onboarding mặc định theo hình dạng PLAN B.3 — tenant chưa từng mở wizard.
  function onboardMacDinh() {
    return {
      buoc: 'chua_kich_hoat', boQuaLuc: null,
  ...
  const CD1 = {
    ...
    orders: ganThueSan(cd1Rev.orders.concat([
    ...
    messages: [
      ...
      // khách im lặng từ 05/08 — quá 7 ngày so với TODAY 17/08, demo cửa sổ tin [Q-005]
      { id:'TN3', date:'2026-08-05', tu:'Chị Sáu (tạp hoá Phù Cát)', noiDung:'Cho tui dat 20 chai nuoc mam nhii gui qua Phu Cat nhe', kenh:'zalo', daTraLoi:true, traLoi:'Dạ em gửi chiều mai chuyến xe Phù Cát, chị nhận giúp em.', traLoiLuc:'2026-08-05' },
    ],
    taxFiled: ['Q1/2026', 'Q2/2026'],

    /* ─── v4: onboarding (PLAN B.3) — CD1 ĐANG NỐI DỞ, trạng thái rải cố ý [P2] ─── */
    license: { ma: maSuat(), loai: 'chuong-trinh', capLuc: '2026-05-12T08:40' },
    vuotLuc: '2026-09-30', // ngày cuối kỳ tính thuế (giả định quý — Q-023) có lũy kế vượt 1 tỷ [Q-001]
    onboarding: {
      buoc: 'dang_noi',
      boQuaLuc: null,
      traLoi: { nguoiLamChinh: 'con', nganh: 'dac-san', giayTo: 'co-mst',
                kenh: ['quay', 'b2b', 'san', 'zalo'], // đúng kênh seed — 3 sàn gộp mã câu OB-2 'san'
                doanhThuUoc: 'tren-1-ty', posHienTai: 'giay-so' },
      ketNoi: {
        bank:   { trangThai: 'da_ket_noi', batDauLuc: '2026-08-16' }, // tín hiệu: webhook thử SePay [Q-002]
        zalooa: { trangThai: 'cho_duyet', batDauLuc: '2026-08-16', hanDuKien: '2026-08-18', // 16/08 là CN → +2 ngày làm việc [Q-004]
                  hoSoDaNop: ['don-dang-ky-zalo-oa', 'anh-gpkd'], aiBam: 'Bà Nguyễn Thị Bảy' },
        hddt:   { trangThai: 'cho_duyet', batDauLuc: '2026-08-16', hanDuKien: '2026-08-17', // cổng miễn phí 15 phút–1 ngày làm việc [Q-002]
                  hoSoDaNop: ['don-dang-ky-hddt', 'anh-gpkd'], aiBam: 'Chị Trần Thu Hà' }, // kế cận seed — hộ >1 tỷ phải đăng ký trong 30 ngày [Q-001]
        cts:    { trangThai: 'dang_dang_ky', batDauLuc: '2026-08-16' }, // đang điền/gửi dở
        shopee: { trangThai: 'chua_hoi' },
        tiktok: { trangThai: 'chua_hoi' },
        lazada: { trangThai: 'chua_hoi' },
      },
      coCanBo: { maCanBo: 'CB-02', ten: '(cán bộ CB-02)', diaBan: 'Quy Nhơn' },
      viecDauTien: { loai: null, batDauLuc: null, doneLuc: null },
    },
  };
  ...
  const CD2 = {
    ...
    /* ─── v4: CD2 giữ onboarding mặc định TRỐNG + license null [P2] — tenant demo
           chạy wizard từ đầu (OB-1 → OB-2 → OB-3 ngay trong buổi xem) ─── */
    license: null,
    onboarding: onboardMacDinh(),
  };
  ...
  const CD3 = {
    ...
    orders: ganThueSan(cd3Rev.orders.concat([
    ...
    /* ─── v4: onboarding CD3 ĐÃ XONG VIỆC ĐẦU [P2] — đích đo R-A2-07 ─── */
    license: { ma: maSuat(), loai: 'chuong-trinh', capLuc: '2026-07-08T07:55' },
    onboarding: {
      buoc: 'xong_viec_dau',
      boQuaLuc: null,
      traLoi: { nguoiLamChinh: 'con', nganh: 'nong-san', giayTo: 'co-mst',
                kenh: ['b2b', 'san', 'live', 'zalo'], // đúng kênh seed — shopee/tiktok gộp mã 'san'
                doanhThuUoc: 'duoi-1-ty', posHienTai: 'giay-so' },
      ketNoi: {
        bank:   { trangThai: 'da_ket_noi', batDauLuc: '2026-07-08' },
        zalooa: { trangThai: 'da_ket_noi', batDauLuc: '2026-07-08' },
        shopee: { trangThai: 'da_ket_noi', batDauLuc: '2026-07-09' }, // đơn sàn đã chảy — thuế sàn nộp thay được ghi nhận [Q-019]
        tiktok: { trangThai: 'da_ket_noi', batDauLuc: '2026-07-09' },
      },
      coCanBo: null,
      // 2 mốc thật cách 14 phút — Trạm hiện «từ mở app tới việc đầu: 14 phút» (B.10)
      viecDauTien: { loai: 'ban-va-bang-ke', batDauLuc: '2026-07-10T09:12', doneLuc: '2026-07-10T09:26' },
    },
  };

  /* ════════════════════════════════════ TENANT TRẮNG ══════════════════════
     cd4-moi [P2] — hộ mới vào Chương trình, CHƯA kích hoạt, chưa nhận diện:
     demo «từ số 0» — mở app thấy đúng OB-1, mọi số liệu = 0. Không skus/orders,
     không keCan (chưa ai nhận diện) — các view duyệt tenant phải tự guard.      */

  const CD4 = {
    id: 'cd4-moi',
    name: 'Hộ mới — chưa nhận diện',
    license: null,
    onboarding: onboardMacDinh(),
  };
  ...
  global.SM_SEED_GIALAI = {
    tenants: { cd1: CD1, cd2: CD2, cd3: CD3, 'cd4-moi': CD4 },
    meta: META,
  };
})(window);
